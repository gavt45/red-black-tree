...
#define NIL &leaf
rbnode leaf = {-1, BLACK, nullptr, nullptr, NIL, NIL};
rbnode *root = NIL;
...
void printConnections(rbnode *n){
    ...
    while (!s.empty()){
        rbnode x = s.top();
        cout << x.key << (x.color == RED ? "r" : "b") << " -right-> " << x.right->key << (x.right->color == RED ? "r" : "b") << endl;
        cout << x.key << (x.color == RED ? "r" : "b") << " -left--> " << x.left->key << (x.left->color == RED ? "r" : "b") << endl;
        s.pop();
        if (x.left != NIL)
            s.push(*x.left);
        if (x.right != NIL)
